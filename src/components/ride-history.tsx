...
export default function RideHistory() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
        ...
        try {
            ...
            // Fetch past rides
            const ridesQuery = query(collection(db, 'rides'), where('passenger', '==', userDocRef));
            ...
            const enrichedRidesPromises = ridesList.map(async (ride) => {
                ...
                return driver ? { ...ride, driver, passenger: user as any } as EnrichedRide : null;
            });
            
            const fetchedRides = (await Promise.all(enrichedRidesPromises)).filter(r => r !== null) as EnrichedRide[];
            setRides(fetchedRides.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()));

            // Fetch scheduled rides
            const scheduledRidesQuery = query(collection(db, 'scheduledRides'), where('passenger', '==', userDocRef));
            const scheduledSnapshot = await getDocs(scheduledRidesQuery);
            const fetchedScheduledRides = scheduledSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as ScheduledRide));
            setScheduledRides(fetchedScheduledRides.sort((a, b) => new Date(b.scheduledTime).getTime() - new Date(a.scheduledTime).getTime()));
  ...
  if (!rides.length && !scheduledRides.length) {
    return (
      <p className="text-muted-foreground text-center">No se encontraron viajes.</p>
    );
  }
              ...
              <div className="space-y-4 pr-4">
                {scheduledRides.map((ride) => (
                  <Card key={ride.id} className="shadow-none border">
                    <CardHeader className="flex flex-row items-center justify-between p-3 border-b">
                        <p className="text-xs text-muted-foreground">
                             {format(new Date(ride.createdAt), "dd 'de' MMM, yyyy", { locale: es })}
                        </p>
                        <Badge variant={ride.status === 'pending' ? 'outline' : 'default'}>
                            {ride.status === 'pending' ? 'Pendiente' : 'Confirmado'}
                        </Badge>
                    </CardHeader>
                     <CardContent className="p-4 space-y-4">
                        <div className="space-y-3 text-sm">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="font-semibold">{format(new Date(ride.scheduledTime), "eeee, dd 'de' MMMM", { locale: es })}</span>
                            </div>
                             <div className="flex items-center gap-3">
                                <Clock className="h-4 w-4 mt-0.5 text-primary"/>
                                <span className="font-semibold">{format(new Date(ride.scheduledTime), "HH:mm 'hrs'", { locale: es })}</span>
                            </div>
                        </div>
                         <Separator />
                        <div className="space-y-3 text-sm">
